...
function getAttributeValue(attributes: ProductAttribute[], slug: string): string | null {
...
function getAttributeValues(attributes: ProductAttribute[], slug: string): string[] {
    const attr = attributes.find((a) => a.attribute.slug === slug);
    if (attr && attr.values.length > 0) {
...
function transformToListItem(product: SaleorProduct): PropertyListItem {
    ...
    const rentalYield = getAttributeValue(product.attributes, 'rental-yield') || '3-5% IRR';
    ...
    try {
        if (product.description) {
            ...
            if (parsed.blocks && Array.isArray(parsed.blocks)) {
                ...
                if (listBlock && listBlock.data && Array.isArray(listBlock.data.items)) {
    ...
    return {
        ...
        status,
        features,
        image: product.media?.[0]?.url || DEFAULT_PROPERTY_IMAGES[product.slug] || DEFAULT_PROPERTY_IMAGE,
...
function transformToDetail(product: SaleorProduct): PropertyDetail {
    ...
    const status = getAttributeValue(product.attributes, 'status') || 'Ready to Move';
    const propertyType = product.productType?.name || 'Residential';
    const totalArea = getAttributeValue(product.attributes, 'total-area') || '';
    const configuration = getAttributeValue(product.attributes, 'configuration') || '';
    const possession = getAttributeValue(product.attributes, 'possession') || 'Immediate';
    const rentalYield = getAttributeValue(product.attributes, 'rental-yield') || '3.5% p.a.';
    const appreciation = getAttributeValue(product.attributes, 'appreciation') || '15-20%';
    const furnishing = getAttributeValue(product.attributes, 'furnishing') || 'Semi-Furnished';
    ...
    return {
        ...
        status,
        heroImage: product.media?.[0]?.url || DEFAULT_PROPERTY_IMAGES[product.slug] || DEFAULT_PROPERTY_IMAGE,
        ...
        amenities: getAttributeValues(product.attributes, 'amenities').map(name => {
            const lower = name.toLowerCase();
            let icon = 'star';
            let description = 'Premium amenity';

            if (lower.includes('pool')) { icon = 'pool'; description = 'Luxury pool with city views.'; }
            else if (lower.includes('fitness') || lower.includes('gym')) { icon = 'fitness_center'; description = 'State-of-the-art equipment.'; }
            else if (lower.includes('spa') || lower.includes('wellness')) { icon = 'spa'; description = 'Relaxation and wellness center.'; }
            else if (lower.includes('security')) { icon = 'security'; description = '24/7 advanced security.'; }
            else if (lower.includes('concierge')) { icon = 'concierge'; description = 'Dedicated concierge service.'; } // 'concierge' icon might not exist in standard material, but trying. Or 'room_service'
            else if (lower.includes('parking')) { icon = 'local_parking'; description = 'Secure private parking.'; }
            else if (lower.includes('smart')) { icon = 'smart_toy'; description = 'Integrated smart home features.'; } // or 'settings_remote'
            else if (lower.includes('garden') || lower.includes('green')) { icon = 'yard'; description = 'Landscaped green spaces.'; }
...
export function useProperties() {
    return useQuery({
        queryKey: ['properties'],
        queryFn: async () => {
            const data = await graphqlRequest<ProductsResponse>(PRODUCTS_QUERY, { first: 50 });
...
export function useProperty(slug: string) {
    return useQuery({
        ...
        queryFn: async () => {
            const data = await graphqlRequest<ProductBySlugResponse>(PRODUCT_BY_SLUG_QUERY, { slug });
            if (!data.product) {
...
function transformToConstructionAsset(product: SaleorProduct): ConstructionAsset | null {
    const assetMeta = product.metadata?.find((m: any) => m.key === "construction_asset");
